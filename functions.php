...
function ntt__kid_ntt__function__site_icon() {
    ...
    if ( ! has_site_icon() ) {
        ...
        $preg_match_svg = preg_match( '/(<svg)([^<]*|[^>]*)/', $get_theme_mod_icon );

        if ( $get_theme_mod_icon && $preg_match_svg ) {
            echo '<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,'. base64_encode( $get_theme_mod_icon ).'">';
        } else if ( ! $preg_match_svg ) {
